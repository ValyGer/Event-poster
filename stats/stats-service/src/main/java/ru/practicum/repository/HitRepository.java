...
public interface HitRepository extends JpaRepository<Hit, Long> {
    ...
    @Query("select new ru.practicum.dtoSer.ApplicationDtoForHits(h.app.app, h.uri, count (distinct h.ip))" +
            "from Hit h " +
    ...
    @Query("select new ru.practicum.dtoSer.ApplicationDtoForHits(h.app.app, h.uri, count (h.ip))" +
            "from Hit h " +
            "where h.timestamp between ?1 and ?2 " +
            "group by h.app.app, h.uri " +
